...
export function ItineraryWidget({ itinerary, onConfirm }: ItineraryWidgetProps) {
    ...
    return (
        <motion.div
            ...
            className="bg-slate-800 rounded-xl p-4 border border-slate-700 max-w-2xl"
        >
            <div className="flex items-center gap-2 mb-4">
                <Sparkles size={16} className="text-emerald-400" />
                <span className="text-sm font-medium text-slate-300">Your Itinerary</span>
            </div>

            <div className="space-y-4">
                {days.map((day) => (
                    <div key={day.day} className="bg-slate-700/50 rounded-lg p-3">
                        {/* Day Header */}
                        ...
                        <div className="space-y-2 ml-4 border-l-2 border-slate-600 pl-4">
                            {day.places.map((place, idx) => (
                                <div key={idx} className="relative">
                                    {/* Timeline dot */}
                                    <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full bg-emerald-500 border-2 border-slate-700" />
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.99 }}
                onClick={onConfirm}
                className="w-full mt-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-lg font-medium flex items-center justify-center gap-2 hover:from-emerald-600 hover:to-teal-700 transition-all"
            >
                <Check size={18} />
                Save this Itinerary
            </motion.button>
